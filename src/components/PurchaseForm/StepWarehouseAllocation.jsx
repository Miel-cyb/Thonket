...
const buildAllocationsMap = (updatedItems) => {
  ...
  updatedItems.forEach((item, index) => {
    ...
    (item.allocations || []).forEach((a) => {
      if (a.warehouseId) {
        itemMap[a.warehouseId] = parseInt(a.qty, 10) || 0;
      }
    });
    map[id] = itemMap;
  });
...
export default function StepWarehouseAllocation({
  ...
  // High performance updates (Syncs both items and top-level allocations map)
  ...
  // Filter engine for items (Preserving master index reference)
  ...
  return (
    <div className="space-y-4 sm:space-y-6 w-full max-w-full mx-auto p-2 sm:p-6 font-sans text-slate-800 antialiased box-border overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-2 mb-2 min-w-0">
                  ...
                  <span
                    className={`text-xs font-mono font-bold shrink-0 ${
                      isOverflow ? "text-rose-600" : "text-slate-700"
                    }`}
                  >
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            ...
            {[
              ...
              <button
                ...
                className={`px-2.5 py-1.5 sm:py-1 rounded-lg font-semibold transition-colors cursor-pointer text-xs ${
                  statusFilter === btn.id
              ...
              return (
                <div key={itemKey} className="p-4 space-y-3">
                  ...
                  <button
                    ...
                    className={`w-full inline-flex items-center justify-center gap-1.5 font-bold text-xs px-3 py-2 rounded-lg border transition-all cursor-pointer shadow-sm ${
                      isExpanded
                  ...
                  >
                    ...
                    <span>{isExpanded ? "Close Routing" : "Configure Routing"}</span>
                    <ChevronDown
                  ...
                  {isExpanded && renderItemConfigPanel(item, itemKey, allocations, stats)}
                </div>
              );
            })
          )}
        ...
        {/* RESPONSIVE LAYOUT 2: TABLET/DESKTOP TABLE VIEW (>= md) */}
          ...
          <table className="w-full text-left border-collapse table-auto">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-100/80 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-600">
                ...
                <th className="p-3.5 text-right whitespace-nowrap">Target Qty</th>
                <th className="p-3.5 text-right whitespace-nowrap">Allocated Qty</th>
                <th className="p-3.5 whitespace-nowrap">Allocation Status</th>
                <th className="p-3.5">Destinations</th>
                <th className="p-3.5 text-right pr-4 sm:pr-5 whitespace-nowrap">Actions</th>
              </tr>
            </thead>
                ...
                filteredItems.map(({ item, key: itemKey }) => {
                  ...
                  return (
                    <React.Fragment key={itemKey}>
                      ...
                      >
                        <td className="p-3.5 pl-4 sm:pl-5 font-medium max-w-[240px]">
                        ...
                        <td className="p-3.5 text-right pr-4 sm:pr-5 whitespace-nowrap">
                          <button
                            type="button"
                            onClick={() =>
                              setExpandedItem(isExpanded ? null : itemKey)
                            }
                            className={`inline-flex items-center gap-1.5 font-bold text-xs px-3 py-1.5 rounded-lg border transition-all cursor-pointer shadow-sm ${
                              isExpanded
                                ? "bg-indigo-600 text-white border-indigo-700"
                                : "bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50"
                            }`}
                          >
                            <SlidersHorizontal className="w-3.5 h-3.5" />
                            <span>{isExpanded ? "Close Routing" : "Configure"}</span>
                            <ChevronDown
                              className={`w-3.5 h-3.5 transition-transform duration-200 ${
                                isExpanded ? "rotate-180" : ""
                              }`}
                      ...
                      {isExpanded && (
                        <tr>
                          <td colSpan={6} className="p-3 bg-indigo-50/20">
