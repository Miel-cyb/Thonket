...
//This is the form where you input customer orders
export default function OrderEntryForm({ onCancel, onCreateOrder, customers, agent }) {
    ...
    const fetchPriceCatalog = useCallback(async (params = {}) => {
        setLoading(true);
        try {
            const { data } = await axios.get(`${API_ENDPOINTS.CATALOG}/${organizationId}/full`, { params });
            ...
            // Extract raw product variants from the API response
            const rawProducts = data?.products || data?.data?.products || [];

            // Transform the variant-level items into a clean UI-friendly structure
            const transformedProducts = rawProducts.map(item => {
                const productDetails = item.productId || {};

                return {
                    _id: item._id,
                    parentProductId: productDetails._id || "",
                    name: item.name || productDetails.name,
                    parentProductName: productDetails.name || "",
                    sku: item.sku || "",
                    slug: productDetails.slug || "",
                    description: productDetails.description || "",
                    categoryId: productDetails.categoryId?._id || item.price?.categoryId || "",
    ...
    const addToCart = (product, variant, initialQty = 1) => {
        ...
        const productPriceObj = product.productPrices?.[0] || product.prices?.[0] || variant.price;
        const variantPriceObj = variant.price || productPriceObj;
    ...
    return (
        <div className="h-full flex flex-col bg-slate-100/80 font-sans text-slate-900 overflow-hidden relative">
            {step === 1 && (
                <header className="px-6 lg:px-8 py-3 bg-white/90 backdrop-blur-xl border-b border-slate-200/80 shadow-[0_4px_20px_-4px_rgba(15,23,42,0.04)] shrink-0 z-20 flex items-center gap-4">
                    <button
                        onClick={onCancel}
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative">
                {step === 1 ? (
                    <>
                        <div className="bg-white/80 backdrop-blur-md border-b border-slate-200/80 shadow-2xs shrink-0 z-10 transition-all">
                            <div className="px-6 lg:px-8 py-3 flex gap-2.5 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] border-b border-slate-100 items-center">
                            ...
                            {subCategories.length > 0 && (
                                <div className="px-6 lg:px-8 py-2.5 bg-slate-50/70 flex gap-2 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] animate-in slide-in-from-top-2 duration-300 items-center">
                        ...
                        <div className={`flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-4 lg:p-6 scroll-smooth ${cart.length > 0 ? 'pb-44' : 'pb-12'}`}>
                            {loading ? (
                    ...
                    <div className="h-full p-4 lg:p-8 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] bg-gradient-to-br from-slate-50 via-slate-100/60 to-indigo-50/30">
                ...
                {step === 1 && cart.length > 0 && (
                    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-full max-w-fit px-4 pointer-events-none animate-in fade-in slide-in-from-bottom-8 duration-500">
                        <button
                            onClick={() => setStep(2)}
                            className="pointer-events-auto flex items-center gap-5 md:gap-7 bg-slate-900 text-white p-2.5 pl-7 pr-2.5 rounded-full shadow-[0_20px_50px_rgba(15,23,42,0.35)] hover:bg-indigo-600 transition-all hover:-translate-y-1.5 active:scale-95 group border border-white/15 backdrop-blur-md"
                        >
                            ...
                            <div className="bg-white/10 group-hover:bg-white/20 h-11 px-5 md:px-6 rounded-full flex items-center gap-3 transition-all">
                                ...
                                <div className="bg-white text-slate-900 rounded-full p-2 group-hover:translate-x-1 transition-transform shadow-md">
...
function MainCatBtn({ label, active, onClick, count }) {
    return (
        <button
            onClick={onClick}
            className={`px-4.5 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all whitespace-nowrap flex items-center gap-2 shadow-2xs ${active
                ? "bg-slate-900 text-white shadow-lg scale-[1.02] ring-2 ring-slate-900/20"
                : "bg-white border border-slate-200/80 text-slate-600 hover:bg-slate-50 hover:border-slate-300"
                }`}
        >
            <span>{label}</span>
            {count !== undefined && (
                <span className={`px-2 py-0.2 rounded-full text-[9px] ${active ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'}`}>
                    {count}
                </span>
            )}
        </button>
...
function SubCatBtn({ label, active, onClick }) {
    return (
        <button
            onClick={onClick}
            className={`px-3.5 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all whitespace-nowrap shadow-2xs ${active
